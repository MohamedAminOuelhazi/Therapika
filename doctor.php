...
<?php
    ...
    else
    {
        ...
        $sql ="INSERT INTO doctor(doctorname,mobileno,departmentid,loginid,password,status,education,experience,nomImage,tailleImage,typeImage,binImage,consultancy_charge) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $req = $conn->prepare($sql);
        $req->execute(array($_POST['doctorname'], $_POST['mobilenumber'], $_POST['department'], $_POST['loginid'], $pass, $_POST['status'], $_POST['education'], $_POST['experience'], $_FILES["image"]["name"],$_FILES["image"]["size"],$_FILES["image"]["type"],file_get_contents($_FILES["image"]["tmp_name"]), $_POST['consultancy_charge']));
        ...
        <div class="col-sm-4">
            <select class="form-control" name="department" id="department" placeholder="Enter lastname...." required="">
                ...
                <?php
                    ...
                    {
                    if(isset($rsedit['departmentid']) && $rsdepartment['departmentid'] == $rsedit['departmentid'])
                    {

...
<section class=" justify-content-center justify-content-lg-between p-4 border-bottom">
    ...
    <div class="carousel-inner">
        ...
        <div class="card ">
            <img src="img/img9.jpg" class="d-block w-100" alt="1">
            <div class="card-img-overlay text-center " style="padding-top: 200px;">


              <h5 class="card-title title" style=" margin-bottom: 10px ; color:#11b2bb">les Médecins</h5>
...
<section class=" justify-content-center justify-content-lg-between p-4 border-bottom">
    ...
    $query = "SELECT * FROM doctor "; // Sélectionner le médecin avec l'ID spécifié
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()){
            $imageName = $row['image']; // Nom de l'image
            $imageData = $row['binImage']; // Contenu de l'image (binImage dans votre base de données)
            $doctorname = $row['doctorname'];
            $education = $row['education'];
            $experience = $row['experience'];
            ?>
            <div class="col" >
            <div class="card h-100 .card-horizontal" style="width: 27rem;">
                <?php   
                echo '<img src="data:image/jpeg;base64,' . base64_encode($imageData) . '" class="card-img-top" alt="' . $imageName . '">' . PHP_EOL;
                ?>
                <div class="card-body">
                    <?php 
                    echo '<h5 class="card-title">'.$doctorname.'</h5>'  ;
                    echo '<p class="card-text" >Education : '.$education.'</p>';
                    echo '<p class="card-text" >Experience : '.$experience.'</p>'
                    ?>      
                </div>
            </div>
            </div>
